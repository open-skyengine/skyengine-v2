...
- `1204` 接受单字节逻辑卷 `C`、`X`、`Y` 或 `Z`，并把这些应用逻辑卷解析到当前
  ...
  该逻辑卷解析命令混用。
- `1223` 是本地 `sanguo` fixture 退出路径使用的可选平台状态查询。基线 profile
  不提供该状态，接受空输入以及可选的输出/输出长度字段，清空非空输出字段并返回
  `-1`，由调用方继续其回退和退出流程。
...
旧版公共 `advbar.mrp` 引导器是包探测的一个已验证例外：文件缺失时它以空句柄为
`simpleDownload` 启动信号。运行时只对该公共插件名返回 `0`；其他 MRP 探测仍返回
`MR_FAILED`，避免把普通插件缺失误判成损坏包并启动错误的下载流程。
