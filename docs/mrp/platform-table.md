...
| 槽 | 种类 | ABI 名称 | 作用 | 版本 |
|---:|---|---|---|---|
| 40 | 函数 | `mr_open` | 打开文件；失败返回空句柄 `0` | 基础 |
...
`mrc_open` 与其他返回 `MR_FAILED` 的文件 API 不同，使用 NULL 风格的空句柄 `0`
表示失败。宿主文件后端可以在内部使用负数错误值，但平台表适配层必须对所有路径和
文件类型统一转换为 `0`，不能根据文件名或扩展名改变 ABI。
