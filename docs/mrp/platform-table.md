...
基线 profile 已验证的 `mr_plat` 命令包括 `1302`：参数为 `0..5` 的多媒体音量级别，
成功返回 `0`。无音频输出的 headless 后端仍接受该状态设置，但不会产生宿主声音。
...
文件路径语法、根目录和权限由移植层定义。对短信和呼叫等有外部副作用的接口，现代宿主应在平台策略层取得用户授权；这不改变 guest 可见的 ABI 槽位。
