...
- `1206(0)` 初始化 motion provider，`4002(0)` 查询该 provider，`4005(2)` 选择已验证的
  事件模式，`4003(0)` 在用户拒绝动感功能或离开该模式时停用事件。确定性 headless 后端
  不主动产生样本；Flutter/C ABI 宿主可在事件模式激活后投递三轴样本，native guest 通过
  `MR_MOTION_EVENT` 的参数结构读取三个有符号 32 位轴值；
...
- `2043` 的本地调用紧跟已验证的 `2023` 请求；白名单全空参数形式，以及本地游戏
  fixture 命中目标时使用的 12 字节全零播放器选项。该调用启动已准备的播放器；宿主
  在 `2023` 阶段已经开始播放，因此这里保持当前音轨并返回 `0`。任何非零选项、输出
  或回调非空的形态保持 Unsupported。
...
| 槽 | 种类 | ABI 名称 | 作用 | 版本 |
|---:|---|---|---|---|
| 40 | 函数 | `mr_open` | 打开文件；数据文件失败返回 0，MRP 包探测失败保留 `-1` | 基础 |
...
菜单选择事件在 guest 回调结束后自动离开活动输入层，菜单句柄仍可由 guest 刷新或释放。
若回调直接绘制了新的 guest 画面（例如 `white.mrp` 进入棋盘），平台保留该画面；若回调
没有重绘（例如棋局内的“继续游戏”），平台恢复菜单显示前的画面。回调同步创建子菜单、
对话框或文本查看器时，仍按模态 UI 栈处理，不执行这一步自动恢复。

基线 headless profile 已验证文本查看器类型 `1` 和 `2`。仓内 `white.mrp` 使用类型 `1`
显示软件注册提示，平台绘制“确定/返回”软键并分别投递 `MR_DIALOG_EVENT` 结果 `0/1`；
类型 `2` 用于游戏介绍等只读内容，仅绘制“返回”并投递结果 `1`。guest 收到结果后调用
