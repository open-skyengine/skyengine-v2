...
impl GuestMemory {
    ...
    pub fn read_u16(&self, address: GuestAddr) -> Result<u16> {
        let bytes: [u8; 2] = self
            .range(address, 2, Permissions::READ)?
            .try_into()
    ...
    pub fn read_u32(&self, address: GuestAddr) -> Result<u32> {
        let bytes: [u8; 4] = self
            .range(address, 4, Permissions::READ)?
            .try_into()
    ...
    pub fn fetch_u16(&self, address: GuestAddr) -> Result<u16> {
        let bytes: [u8; 2] = self
            .range(address, 2, Permissions::EXECUTE)?
            .try_into()
            .expect("checked two-byte fetch");
        Ok(u16::from_le_bytes(bytes))
    }

    pub fn fetch_u32(&self, address: GuestAddr) -> Result<u32> {
        let bytes: [u8; 4] = self
            .range(address, 4, Permissions::EXECUTE)?
            .try_into()
            .expect("checked four-byte fetch");
        Ok(u32::from_le_bytes(bytes))
    }

    pub fn read(&self, address: GuestAddr, len: usize) -> Result<Vec<u8>> {
        Ok(self.range(address, len, Permissions::READ)?.to_vec())
    }
    ...
    pub fn write_u16(&mut self, address: GuestAddr, value: u16) -> Result<()> {
        self.range_mut(address, 2, Permissions::WRITE)?
            .copy_from_slice(&value.to_le_bytes());
        Ok(())
    }

    pub fn write_u32(&mut self, address: GuestAddr, value: u32) -> Result<()> {
        self.range_mut(address, 4, Permissions::WRITE)?
            .copy_from_slice(&value.to_le_bytes());
        Ok(())
    }

    pub fn write(&mut self, address: GuestAddr, bytes: &[u8]) -> Result<()> {
        self.range_mut(address, bytes.len(), Permissions::WRITE)?
